...
function post(path, params, callback) {
  return state => {
    ...
    return _languageCommon.http.post(config)(state).catch(error => handleError(error)).then(response => handleResponse(state, response)).then(nextState => handleCallback(nextState, callback));
  };
}
/**
 * Make a PUT request
