...
_http.__axios.interceptors.response.use(function (response) {
  let cookies;
  if (response.headers['set-cookie'] instanceof Array) cookies = response.headers['set-cookie'].map(Cookie.parse);else cookies = [Cookie.parse(response.headers['set-cookie'])];
  let keepCookies = [];
